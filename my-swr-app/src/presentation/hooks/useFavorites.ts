...
//   isFavorite: (heroId: number | string) => boolean;
// };
...
//   const isFavorite = (heroId: number | string) => data.some(f => f.id === Number(heroId));

//   const toggleFavorite = async (hero: Hero) => {
//     const isCurrentlyFavorite = isFavorite(hero.id);
